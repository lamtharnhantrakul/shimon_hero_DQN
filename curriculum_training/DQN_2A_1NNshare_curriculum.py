...
def trainNetwork(model):
    ...
    for index, level in enumerate(levels):
        game2 = sh.Game({'USE_MIDI': True, 'MIDI_FILES_DIRECTORY': os.path.join(CURRICULUM_DIRECTORY, level),
        ...
        tot_notes = game2.tot_notes
        print(game2.get_settings())
        max_score = 0
        while(max_score < tot_notes):
            ...
            image_data, r_t, game_over, game_score = game2.next_action([action_dict[L_act_idx], action_dict[R_act_idx]])
            tot_notes = game2.tot_notes
            ...
            if t > OBSERVE:
                ...
                loss = model.train_on_batch({'main_input': inputs}, {'L_output': L_targets, 'R_output': R_targets})[0]  # Select the first element, which is the total loss from L and R arms
            ...
            if game_score > max_score:
                ...
                logging_string = "TIME %8d | LEVEL %2d | MAX_SCORE %3d" % (t, index + 1, max_score)
            ...
            if DEBUG:
                ...
                if t <= OBSERVE:
                    ...
                    state = "Training"
                debugging_string = "TIME %8d | STATE %1s | LEVEL %2d | EPS %3.10f | L_ACT %1d | R_ACT %1d | REW %5.1f | L_Q_MAX_t1 %8.4f | R_Q_MAX_t1 %8.4f | LOSS %8.4f | GAME_O %s | MAX_SCORE %3d" % (
                    t, state, index+1, epsilon, L_act_idx, R_act_idx, r_t, np.max(L_Q_t1), np.max(R_Q_t1), loss, str(game_over), max_score)
