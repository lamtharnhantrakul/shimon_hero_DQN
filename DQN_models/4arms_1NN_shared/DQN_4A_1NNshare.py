...
model_prefix = "4A1NNshared_" + timestr  # The prefix used to identify the model and time training was created
...
def buildmodel():
    ...
    #FC1_out = Dropout(0.5)(FC1_out)

    ### ARM indexing goes arm0, arm1, arm2, arm3
    # ARM 0 (leftmost arm)
    arm0_FC_out = Dense(256,
                     kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None),
                     name='arm0_FC',
                     activation='relu')(FC1_out)

    arm0_output = Dense(NUM_ACTIONS,
                     name='arm0_output',
                     kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None))(arm0_FC_out)

    # ARM 1 (2nd leftmost arm)
    arm1_FC_out = Dense(256,
                     kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None),
                     name='arm1_FC',
                     activation='relu')(FC1_out)

    arm1_output = Dense(NUM_ACTIONS,
                     name='arm1_output',
                     kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None))(arm1_FC_out)

    # ARM 2 (2nd rightmost arm)
    arm2_FC_out = Dense(256,
                        kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None),
                        name='arm2_FC',
                        activation='relu')(FC1_out)

    arm2_output = Dense(NUM_ACTIONS,
                        name='arm2_output',
                        kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None))(arm2_FC_out)

    # ARM 3 (rightmost arm)
    arm3_FC_out = Dense(256,
                        kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None),
                        name='arm3_FC',
                        activation='relu')(FC1_out)

    arm3_output = Dense(NUM_ACTIONS,
                        name='arm3_output',
                        kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None))(arm3_FC_out)
    ...
    model = Model(inputs=input_img, outputs=[arm0_output, arm1_output, arm2_output, arm3_output])
...
def trainNetwork(model):
    ...
    image_data, r_t, game_over, game_score = game.next_action([0,0,0,0])
    ...
    while (True):
        ...
        if t % FRAME_PER_ACTION == 0:  # This selects an action depending on FRAME_PER_ACTION (default = 1)
            if random.random() <= epsilon:
                arm0_act_idx = random.randrange(NUM_ACTIONS)
                arm1_act_idx = random.randrange(NUM_ACTIONS)
                arm2_act_idx = random.randrange(NUM_ACTIONS)
                arm3_act_idx = random.randrange(NUM_ACTIONS)
            else:
                ...
                arm0_act_idx = np.argmax(q_predictions[0])
                arm1_act_idx = np.argmax(q_predictions[1])
                arm2_act_idx = np.argmax(q_predictions[2])
                arm3_act_idx = np.argmax(q_predictions[3])
        ...
        image_data, r_t, game_over, game_score = game.next_action([action_dict[arm0_act_idx], action_dict[arm1_act_idx], action_dict[arm2_act_idx], action_dict[arm3_act_idx]])
        ...
        RM.append((s_t, [arm0_act_idx, arm1_act_idx, arm2_act_idx, arm3_act_idx], r_t, s_t1, game_over))
        ...
        if t > OBSERVE:
            ...
            arm0_targets = np.zeros((BATCH, NUM_ACTIONS))  # This should be (32,3) and stores the r_t for each of the actions for L arm
            arm1_targets = np.zeros((BATCH, NUM_ACTIONS))  # This should be (32,3) and stores the r_t for each of the actions for L arm
            arm2_targets = np.zeros((BATCH, NUM_ACTIONS))
            arm3_targets = np.zeros((BATCH, NUM_ACTIONS))

            # Use random indices to select minibatch from Replay Memory
            for i in range(0, len(mini_batch)):
                state_t = mini_batch[i][0]  # This is the state
                arm0_action_t = mini_batch[i][1][0]
                arm1_action_t = mini_batch[i][1][1]
                arm2_action_t = mini_batch[i][1][2]
                arm3_action_t = mini_batch[i][1][3]
                ...
                predictions_t = model.predict(state_t)
                arm0_targets[i] = predictions_t[0]
                arm1_targets[i] = predictions_t[1]
                arm2_targets[i] = predictions_t[2]
                arm3_targets[i] = predictions_t[3]
                ...
                arm0_Q_t1 = predictions_t1[0]  # This is a 1x3 vector corresponding to probs of each possible action
                arm1_Q_t1 = predictions_t1[1]  # D the same for the right arm
                arm2_Q_t1 = predictions_t1[2]  # D the same for the right arm
                arm3_Q_t1 = predictions_t1[3]  # D the same for the right arm

                # if game_over_t=True, then state equals reward_t
                if game_over_t:
                    arm0_targets[i, arm0_action_t] = reward_t
                    arm1_targets[i, arm1_action_t] = reward_t
                    arm2_targets[i, arm2_action_t] = reward_t
                    arm3_targets[i, arm3_action_t] = reward_t
                # if game is still playing, then action receives a discounted reward_t
                else:
                    arm0_targets[i, arm0_action_t] = reward_t + (GAMMA * np.max(arm0_Q_t1))
                    arm1_targets[i, arm1_action_t] = reward_t + (GAMMA * np.max(arm1_Q_t1))
                    arm2_targets[i, arm2_action_t] = reward_t + (GAMMA * np.max(arm2_Q_t1))
                    arm3_targets[i, arm3_action_t] = reward_t + (GAMMA * np.max(arm3_Q_t1))
            ...
            loss = model.train_on_batch({'main_input': inputs}, {'arm0_output': arm0_targets, 'arm1_output': arm1_targets, 'arm2_output': arm2_targets, 'arm3_output': arm3_targets})[0]  # Select the first element, which is the total loss from L and R arms
        ...
        if SAVE_MODEL and t % 5000 ==0:
            with open(model_log_dir, 'a') as text_file:
                text_file.write("Timestep: " + str(t) + " | Epsilon: " + str(epsilon) + "\n")
        ...
        if DEBUG:
            ...
            if t <= OBSERVE:
                ...
                state = "Training"
            debugging_string = "TIME %8d | STATE %1s | EPS %3.10f | 0_ACT %1d | 1_ACT %1d | 2_ACT %1d | 3_ACT %1d | REW %5.1f | L_Q_MAX_t1 %8.4f | R_Q_MAX_t1 %8.4f | LOSS %8.4f | GAME_O %s | MAX_SCORE %3d" % (
                t, state, epsilon, arm0_act_idx, arm1_act_idx, arm2_act_idx, arm3_act_idx, r_t, np.max(L_Q_t1), np.max(R_Q_t1), loss, str(game_over), max_score)
