# This code awas based on TF 0.8 and Keras 1.0, After 22/3/2017 I migrated to TF 1.0 and Keras 2.0
...
import numpy as np
import shimon_hero as sh  # Using Hanoi's edited version of Shimon Hero
...
action_dict_L = {0: -1, 1: 0, 2: 1}  # Shimon hero will interpret [-1] as one arm left, [1] as one arm right. Or [-1 -1] as two arm left and left.
                                    # So QNN to controls is [1 0 0] = [-1], [0 1 0] = [0], [0 0 1] = [1]
action_dict_R = {0: 1, 1: 0, 2: -1}
...
def trainNetwork(model_L, model_R, args):
    ...
    while (True):
        ...
        if t % FRAME_PER_ACTION == 0:  # This selects an action depending on FRAME_PER_ACTION (default = 1)
            if random.random() <= epsilon:
                ...
                action_index_L = random.randrange(NUM_ACTIONS)
                action_index_R = random.randrange(NUM_ACTIONS)
                #a_t[action_index] = 1  # set action command with correct one-hot encoding
            else:
                #print("--- { NEW ACTION } ---")
                # For the left arm, predict as normal
                q_L = model_L.predict(s_t)  # Inputs the (1,80,80,4) stack of images and outputs a prediction of the best action
                max_Q_L = np.argmax(q_L)  # The index of highest probability in the last Dense layer is the action to take
                action_index_L = max_Q_L.copy()  # The state with max_Q is now the action_index
                #a_t[action_index] = 1  # set action command with correct one-hot encoding

                q_R = model_L.predict(s_t)  # Inputs the (1,80,80,4) stack of images and outputs a prediction of the best action
                max_Q_R = np.argmax(q_R)  # The index of highest probability in the last Dense layer is the action to take
                action_index_R = max_Q_R.copy()  # The state with max_Q is now the action_index
                # a_t[action_index] = 1  # set action command with correct one-hot encoding
        ...
        L_action = [action_dict_L[action_index_L]]
        R_action = [action_dict_R[action_index_R]]

        image_data, r_t, game_over, game_score = game.next_action(np.array([L_action, R_action]))
        ...
        RM.append((s_t, action_index_L, r_t, s_t1, game_over))
        ...
        data_string = "TIME %8d | STATE %1s | EPS %3.10f | ACT %1d | REW %5.1f | Q_MAX_t1 %8.4f | LOSS %8.4f | GAME_O %s | MAX_SCORE %1d" % (t, state, epsilon, action_index_L, r_t, np.max(Q_sa_t1), loss, str(game_over), max_score)
        print (data_string)
...
def playGame(args):
    model_L = buildmodel()
    model_R = buildmodel()
    trainNetwork(model_L, model_R, args)
