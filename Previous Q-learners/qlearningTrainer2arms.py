    ...
    while (True):
        ...
        if t % FRAME_PER_ACTION == 0:  # This selects an action depending on FRAME_PER_ACTION (default = 1)
            if random.random() <= epsilon:
                ...
                action_index_L = max_Q.copy()  # The state with max_Q is now the action_index
                #a_t[action_index] = 1  # set action command with correct one-hot encoding

                # For the right arm, flip the image and query the network for the flipped image
                s_t_flip = np.zeros_like(s_t)
                for i in range(len(s_t)):
                    s_t_flip[i] = np.fliplr(s_t[i])
                    plt.matshow(s_t_flip[i])
                    plt.show()
                max_Q_R = np.argmax(model.predict(s_t_flip))
                action_index_R = max_Q_R.copy()
