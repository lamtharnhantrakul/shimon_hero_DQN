...
def trainNetwork(model, args):
    ...
    while (True):
        ...
        data_string = "TIME %8d | STATE %1s | EPS %3.10f | ACT %1d | REW %5d | Q_MAX_t1 %8.4f | LOSS %8.4f | GAME_O %s " % (t, state, epsilon, action_index, r_t, np.max(Q_sa_t1), loss, str(game_over))
        print (data_string)
