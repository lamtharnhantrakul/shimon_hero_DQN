...
def buildmodel():
    ...
    CNN_model.add(Conv2D(input_shape=(img_rows, img_cols, img_channels),
                         filters=32,
                         kernel_size=(8, 8),
                         strides=(4, 4),
    ...
    L_model = Sequential()
    L_model.add(CNN_model)

    R_model = Sequential()
    R_model.add(CNN_model)
    # LEFT ARM
    L_model.add(Dense(256,
                      kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None),
                      name='L_FC',
                      activation='relu'))

    L_model.add(Dense(NUM_ACTIONS,
                      name='L_output',
                      kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None)))

    # RIGHT ARM
    R_model.add(Dense(256,
                  kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None),
                  name='R_FC',
                  activation='relu'))

    R_model.add(Dense(NUM_ACTIONS,
                  name='R_output',
                  kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=None)))
    ...
    while (True):
        ...
        if t % FRAME_PER_ACTION == 0:  # This selects an action depending on FRAME_PER_ACTION (default = 1)
            if random.random() <= epsilon:
                ...
                L_act_idx = np.argmax(
                    L_Q)  # The index of highest predicuted Q value in the last Dense layer is the action to take

                R_Q = L_model.predict(
                    s_t)  # Inputs the (1,80,80,4) stack of images and outputs a prediction of the best action
                R_act_idx = np.argmax(
                    R_Q)  # The index of highest predicuted Q value in the last Dense layer is the action to take
        ...
        if t > OBSERVE:
            ...
            L_targets = np.zeros(
                (BATCH, NUM_ACTIONS))  # This should be (32,3) and stores the r_t for each of the actions
            ...
            for i in range(0, len(mini_batch)):
                ...
                L_Q_t1 = L_model.predict(
                    state_t1)  # This is a 1x4 vector corresponding to probs of each possible action
        ...
        if DEBUG:
            ...
            debugging_string = "TIME %8d | STATE %1s | EPS %3.10f | L_ACT %1d | R_ACT %1d | REW %5.1f | L_Q_MAX_t1 %8.4f | R_Q_MAX_t1 %8.4f | LOSS %8.4f | GAME_O %s | MAX_SCORE %3d" % (
                t, state, epsilon, L_act_idx, R_act_idx, r_t, np.max(L_Q_t1), np.max(R_Q_t1), loss, str(game_over),
                max_score)
        ...
        if SAVE_MODEL and t % 10000 == 0:
            ...
            save_model(R_model, R_model_path,
                       overwrite=True)  # saves weights, network topology and optimizer state (if any)
