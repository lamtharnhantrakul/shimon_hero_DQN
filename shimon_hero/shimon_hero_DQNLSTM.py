...
class GameSettings(object):
    def __init__(self):
        ...
        self.SCREEN_HEIGHT = int((0.6)*self.SCREEN_WIDTH + self.ARM_HEIGHT)
    ...
    def __init__(
        ...
        if note != None:
            ...
            while self.note < 0:
                self.note += 12
            '''
            self.note = note % gs.TOTAL_NUM_NOTES
        else:
...
class Game(object):
    def __init__(self, *initial_data, **kwargs):
        ...
        self.reward = 0
        self.is_terminal = False  # self.is_terminal is initialized here
        self.count = +1
    ...
    def next_action(self, input_actions):
        ...
        if self.is_terminal:
            self.__init__()  # self.is_terminal is put back to False when init() is called
            # If the player dies, then restart the game. HOWEVER, I need the self.is_terminal=True
        ...
        if gs.DISPLAY_SCORE:
            ...
            self.screen.blit(
                self.score,
                (6, 0))
        ### Next note that is generated
        next_note = 0  # here I know the next note is midi no. 4 by default, need a better way to do this in the future
        ...
        if self.note_count < len(self.midi_notes) - 1:
            while self.midi_notes[self.note_count][1] == self.step_count:
                ...
                print ("Note generated: " + str(note.getNote()))
                next_note = note.getNote()  # When the note is generated, the game outputs this information to the agent
        ...
        return image_data, self.reward, self.is_terminal, score_total, next_note
